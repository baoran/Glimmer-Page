...
function renderSwarmReview(review, kicker = "AGENT SWARM") {
  ...
  return `<details class="swarm-review"><summary><span>${esc(kicker)}</span><b class="${verdict.className}">${verdict.label} · ${arbitration.score} 分</b><em>分歧 ${arbitration.disagreement?.level ?? "—"}</em></summary><div class="swarm-agent-grid">${agents.map((agent) => {
...
function renderForecasts() {
  ...
  const shadowDefinitions = Object.values(data.model.swarmShadows ?? {});
  const swarmSummary = run.swarmSummary;
  const swarmCounts = swarmSummary?.verdictCount ?? { support: 0, neutral: 0, challenge: 0 };
  $("#swarm-system").innerHTML = `<div class="swarm-head"><div><p>READ-ONLY SUPERVISION SWARM</p><h2>七角色 Agent Swarm</h2><small>${esc(swarm?.policy ?? "监督层尚未启用")}</small><small><b>参数证据 ${esc(swarm?.parameterEvidence?.level ?? "—")} · ${esc(swarm?.parameterEvidence?.label ?? "未声明")}</b>：${esc(swarm?.parameterEvidence?.statement ?? "等待参数来源声明")}</small></div><div class="swarm-day-summary"><span>覆盖率<b>${swarmSummary ? `${(swarmSummary.coverageRate * 100).toFixed(0)}%` : "未运行"}</b></span><span>支持 / 中性 / 质疑<b>${swarmCounts.support} / ${swarmCounts.neutral} / ${swarmCounts.challenge}</b></span><span>高分歧<b>${swarmSummary?.highDisagreementCases?.length ?? 0} 个</b></span></div></div><div class="swarm-role-grid">${(swarm?.agents ?? []).map((agent, index) => `<article><span>${String(index + 1).padStart(2, "0")}</span><div><b>${esc(agent.label)}</b><p>${esc(agent.responsibility)}</p></div></article>`).join("")}</div>${shadowDefinitions.map((definition) => { const summary = run.shadowSwarmSummaries?.[definition.version]; return `<div class="swarm-shadow-note"><div><span>SHADOW MODEL</span><b>${esc(definition.label)}</b><p>${esc(definition.parameterEvidence?.level)} · ${esc(definition.parameterEvidence?.label)}。${esc(definition.parameterEvidence?.statement)}</p></div><strong>${summary ? `本日 ${summary.reviewedCount} 个并行样本` : "等待下一新交易日，不回填旧日志"}</strong></div>`; }).join("")}<div class="swarm-guide"><b>如何使用</b><p>先看正式 Vector 分数与周期排名，再展开候选卡片中的 Swarm：<strong>支持</strong>表示监督证据较一致，<strong>中性</strong>表示信息有限，<strong>质疑</strong>表示数据或风险 Agent 触发反证。研究 v2 只做 shadow 并行比较，不会自动换股、改变正式分数或训练经验池。</p></div>`;
  const horizonName = new Map(data.horizons.map((item) => [item.id, item.label]));
