...
const shadowDefinitions = data.model.swarmShadows ?? {};
const predictionIds = new Set();
let swarmReviewedPredictionCount = 0;
let shadowSwarmReviewedPredictionCount = 0;
for (const run of data.runs) {
  ...
  for (const prediction of run.predictions) {
    ...
    for (const [version, review] of Object.entries(prediction.shadowSwarmReviews ?? {})) {
      const definition = shadowDefinitions[version];
      if (!definition || review.swarmVersion !== version || definition.shadowOnly !== true || definition.trainingEligible !== false) fail(`Shadow Swarm 版本未声明：${prediction.id}`);
      if (review.formalScoreRef !== prediction.score || review.formalModelVersion !== prediction.modelVersion || review.dataCutoffTradeDate !== prediction.entryCloseDate) fail(`Shadow Swarm 正式模型引用错误：${prediction.id}`);
      if (review.shadowOnly !== true || review.trainingEligible !== false || review.auditTrail?.nonInterference !== true || review.arbitration?.nonInterference !== true) fail(`Shadow Swarm 隔离约束无效：${prediction.id}`);
      if (!/^[a-f0-9]{64}$/.test(review.auditTrail?.inputHash ?? "")) fail(`Shadow Swarm 输入哈希无效：${prediction.id}`);
      for (const id of specialistIds) {
        const agent = review.agents?.[id];
        if (agent?.agentId !== id || !verdicts.includes(agent.verdict) || !(agent.score >= 0 && agent.score <= 100) || !(agent.confidence >= 0 && agent.confidence <= 1)) fail(`Shadow Swarm ${id} Agent 输出无效：${prediction.id}`);
        if (!Array.isArray(agent.signals) || !Array.isArray(agent.warnings) || !Array.isArray(agent.evidence) || !Array.isArray(agent.referenceIds)) fail(`Shadow Swarm ${id} Agent 证据结构无效：${prediction.id}`);
      }
      if (review.arbitration?.agentId !== "arbitration" || !verdicts.includes(review.arbitration.verdict) || review.arbitration.shadowOnly !== true) fail(`Shadow Swarm 仲裁输出无效：${prediction.id}`);
      shadowSwarmReviewedPredictionCount += 1;
    }
  }
  if (run.swarmVersion) {
  ...
  for (const [version, summary] of Object.entries(run.shadowSwarmSummaries ?? {})) {
    if (!shadowDefinitions[version] || summary.swarmVersion !== version || summary.shadowOnly !== true || summary.trainingEligible !== false) fail(`运行 Shadow Swarm 汇总版本无效：${run.runId}`);
    const reviewed = run.predictions.filter((item) => item.shadowSwarmReviews?.[version]).length;
    if (summary.reviewedCount !== reviewed || summary.coverageRate !== reviewed / run.predictions.length || summary.nonInterference !== true) fail(`运行 Shadow Swarm 汇总不完整：${run.runId}`);
  }
}
...
if (data.audit?.shadowSwarmReviewedPredictionCount !== shadowSwarmReviewedPredictionCount) fail("Shadow Swarm 审计计数与实际监督记录不一致");
...
console.log(`预测数据校验通过：${data.runs.length} 个交易日，${predictionIds.size} 个预测，${data.tracking.filter((item) => item.status === "matured").length} 个已到期，${swarmReviewedPredictionCount} 个 v1 监督，${shadowSwarmReviewedPredictionCount} 个 shadow 监督。`);
